...
**Note on `source_page`:** it's the full absolute catalogue page URL
(`https://books.toscrape.com/catalogue/page-1.html`), not a page number. An
integer has no provenance value — you can't click through it to re-verify
where a record came from. `src/schema.js` rejects a numeric `source_page` as
invalid, and `test/run-tests.js` has a regression test for exactly this.
