...
If I regenerated with a tighter prompt, specifying `DATABASE_URL` as a single connection string and explicit partial-update semantics for PUT, I'd expect the connection-format mismatch and the strict-PUT behavior to disappear, since both stemmed from gaps I left in the original prompt rather than the AI ignoring an instruction.