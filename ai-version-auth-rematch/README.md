...
## Two things that differ from Neon's published docs

Neon's own documentation shows `signUp.email()` and `signIn.email()`
returning `{ data: { user, session: { token, expiresAt, ... } } }`. In
practice, testing against the live API, both calls return `{ data: { user,
token } }` with no `session` object at all. The code in
`auth.controller.js` reads `data.token` directly. If a future SDK version
changes this again, that's the first place to check.

Neon Auth also rejects pure server-to-server calls with `"Origin header is
required"` unless an `Origin` header is attached by hand, even though
there's no browser and nothing to enforce CORS against. `getAuthClient()`
sets this explicitly via `fetchOptions.headers.Origin`, using `APP_ORIGIN`
from `.env`. Set it to wherever this server actually runs.
